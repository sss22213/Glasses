...
namespace Kinect
{
	...
	public:
		...
		Mat Colorframe_BGR()
		{
			//Get BGRA img from Kinect
			Mat Get_Kinect_img_BGRA = *this->Colorframe();
			//Convert BGRA to BGR
			cv::cvtColor(Get_Kinect_img_BGRA, Get_Kinect_img_BGRA, CV_BGRA2BGR);
			return Get_Kinect_img_BGRA;
	...
	public:
		...
		bool Joint_Trig(cv::Point pt1, cv::Point pt2,JointType Inputjoint= JointType::JointType_HandLeft)
		{
			double *temp=pJoint->Get_Joint(Inputjoint);
			if (temp[1] > pt1.x && temp[1]<pt2.x && temp[2]>pt1.y && temp[2] < pt2.y)return true;
			else return false;
		}
		bool Joint_Trig(int x1,int x2,int y1,int y2, JointType Inputjoint = JointType::JointType_HandLeft)
		{
			cv::Point pt1(x1, y1);
			cv::Point pt2(x2, y2);
			double *temp=pJoint->Get_Joint(Inputjoint);
	...
	public:
		...
		HRESULT Image_puts()
		{
			...
			for (int i = 0; it != (PicBox->MyPictureBox_Get())->end(); it++, i++)
			{
				...
				//Tweak color
				addWeighted(ImageROT, 1, logo, 0, 0, ImageROT);
				//Picture
	...
	public:
		...
		cv::Point Trig_Color_func_GPU(Mat Update_background)
		{
			...
			while ((float)sum(binary_roi)[0] / ((float)Trig_One_Range *(float)Trig_One_Range * 255.0) > 0.3)
			{
				if ((maxLoc.x + Trig_One_Range > Trig_RangeX) || (maxLoc.y + Trig_One_Range > Trig_RangeY))
				{
					break;
				}
				binary_roi = b(Rect(maxLoc.x+20, maxLoc.y+20, maxLoc.x + Trig_One_Range, maxLoc.y + Trig_One_Range));
				minMaxLoc(binary_roi, &minVal, &maxVal, &minLoc, &maxLoc);
			}
			/*
			cv::medianBlur(b, b,11);
			double minVal,maxVal = 0.0;
			cv::Point minLoc,maxLoc;
			minMaxLoc(b, &minVal,&maxVal, &minLoc,&maxLoc);
			if ((maxLoc.x + Trig_One_Range < Trig_RangeX) && (maxLoc.y + Trig_One_Range < Trig_RangeY))
			{
				Mat binary_roi = b(Rect(0, 0, maxLoc.x + Trig_One_Range, maxLoc.y + Trig_One_Range));
				if ((float)sum(binary_roi)[0] / ((float)Trig_One_Range *(float)Trig_One_Range * 255.0) > 0.3)
				{
